...
public class DerivedBulletInfo : MonoBehaviour
{
    ...
    public void SetFromPlayer(float shot_angle, bool parent_critical, float crit_damage_p) // 플레이어가 발사한 총알을 부모로할 때
    {
        ...
        {
            crit = true;
            crash_damage = crash_damage * Camera.main.GetComponent<Ingame_manager>().ex_total.ex_crash_dmg * crit_damage_p;
            GetComponent<SpriteRenderer>().color = new Color(255, 0, 0);
        }
        else
        {
            crit = false;
            crash_damage = crash_damage * Camera.main.GetComponent<Ingame_manager>().ex_total.ex_crash_dmg;
        }
    }
